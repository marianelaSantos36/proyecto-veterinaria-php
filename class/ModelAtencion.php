...
class Atencion{
    ...
    public function listarAtencion() {
        try {
            ...
            $prepar = $this->db->prepare("SELECT * FROM atenciones");
    ...
    public function guardarAtencion($datos) {
        try {
            $prepar = $this->db->prepare("INSERT INTO atenciones (mascota_id, doctor_id, fecha, diagnostico, tratamiento) 
                                          VALUES (:mascota_id, :doctor_id, :fecha, :diagnostico, :tratamiento)");
            
            $prepar->bindParam(':mascota_id', $datos["mascota_id"], PDO::PARAM_INT);
            $prepar->bindParam(':doctor_id', $datos["doctor_id"], PDO::PARAM_INT);
            $prepar->bindParam(':fecha', $datos["fecha"], PDO::PARAM_STR);
            $prepar->bindParam(':diagnostico', $datos["diagnostico"], PDO::PARAM_STR);
            $prepar->bindParam(':tratamiento', $datos["tratamiento"], PDO::PARAM_STR);
            
            if ($prepar->execute()) {
                // Redireccionar a home.php con un mensaje de confirmación
                header('Location: ../vistaAtencion/homeAtencion.php');
                exit();
            } else {
